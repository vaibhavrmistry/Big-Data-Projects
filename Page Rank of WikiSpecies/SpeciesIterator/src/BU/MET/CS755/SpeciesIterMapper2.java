 ...
 public class SpeciesIterMapper2 extends MapReduceBase implements Mapper<WritableComparable, Writable, Text, Text> { 
   ...
   public void map(WritableComparable key, Writable value, 
     ...
     try { 
        ...
        currScore = 0.5; // Made changes here
     } 
     ...
     Text toEmit = new Text((new Double(.85 * currScore / numoutlinks)).toString()); // changed damping factor  
     for (String page : pages) { 
       if(page.length() > 0) {
         output.collect(new Text(page), toEmit); 
         //output.collect(new Text(page), new  Text(" " + pagetitle)); //Removed this line
       }
     } 

     // collect the inlink with its dampening factor, and all outlinks
     output.collect(new Text(pagetitle), new Text(".00000064")); //Made Changes here// old .00000211
